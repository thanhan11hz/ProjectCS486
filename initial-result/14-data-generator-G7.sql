...
--      where affected bookings are surfaced for staff contact, never
...
DECLARE @AY1Start DATETIME2 = '2023-09-01';           -- Academic year 2023-24
...
mgen AS (
    SELECT
        ...
        ABS(CHECKSUM(CONCAT('mday', n)))   % 1160       AS day_off,
...
--    record exists, therefore advisory_acknowledged = NULL is correct
--    (BR-45: no advisory was active at booking time).
--
...
cand AS (
    SELECT
        ...
        p.dcf,
        p.bcf,
        CASE WHEN (p.base_pct * p.dcf * p.bcf) / 100 < 4  THEN 4
             WHEN (p.base_pct * p.dcf * p.bcf) / 100 > 97 THEN 97
             ELSE (p.base_pct * p.dcf * p.bcf) / 100 END AS eff_pct,
...
DECLARE @AY2Start DATETIME2 = '2024-09-01';           -- Academic year 2024-25
DECLARE @UserCount INT = (SELECT COUNT(*) FROM users);
DECLARE @StaffCount INT = (SELECT COUNT(*) FROM users WHERE role IN (N'facility_staff', N'facility_manager'));
...
mgen AS (
    SELECT
        ...
        ABS(CHECKSUM(CONCAT('bday', n))) % 730         AS day_off,
...
--     bookings for staff contact (BR-48) and never retroactively rejects
--     existing bookings, so inserting them after all bookings is correct and
--     cannot violate trg_bookings_br44_no_overlap_out_of_service.
-- ============================================================================

DECLARE @AY3Start DATETIME2 = '2025-09-01';
DECLARE @UserCount INT = (SELECT COUNT(*) FROM users);
DECLARE @StaffCount INT = (SELECT COUNT(*) FROM users WHERE role IN (N'facility_staff', N'facility_manager'));
...
mgen AS (
    SELECT
        ...
        ABS(CHECKSUM(CONCAT('cday', n))) % 365         AS day_off,
...
-- ============================================================================
-- 12. DATA INTEGRITY SUMMARY — counts and cross-checks
...
-- BR-46 data-level consistency: every booking with advisory_acknowledged = 1
-- must overlap an open advisory record; every booking overlapping one must
-- carry the acknowledgement.  Expect 0 on both sides.
